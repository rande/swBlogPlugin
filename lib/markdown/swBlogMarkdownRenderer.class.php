...
/**
 * This file come from the sympal project
 * 
 * credit to Jonathan Wage
 * 
 */
require_once('Markdown.php');

class swBlogMarkdownRendered
{
  public static function convertToHtml($markdown)
  {
    ...
    return '<div class="sw_blog_markdown">'.self::enhanceHtml(Markdown($markdown)).'</div>';
  }
  ...
  public static function enhanceHtml($html)
  {
    ...
    // SQL
    $html = preg_replace_callback('#<pre><code>(alter|create|drop|select|update|delete|from|group by|having|where)(.+?)</code></pre>#si', array('swBlogMarkdownRendered', 'highlightSql'), $html);

    // Yaml
    $html = preg_replace_callback('#<pre><code>(.+?)</code></pre>#s', array('swBlogMarkdownRendered', 'highlightYaml'), $html);

    // Syntax highlighting
    $html = preg_replace_callback('#<pre><code>(.+?)</code></pre>#s', array('swBlogMarkdownRendered', 'highlightPhp'), $html);
  ...
  static protected function highlightYaml($matches)
  {
    ...
    if ($formatted = sfBlogYamlSyntaxHighlighter::highlight($yaml))
    {
      return $formatted;
    } else {
      return $matches[0];
